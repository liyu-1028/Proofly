...
- 本文档中对应模块的完成说明。
- 如涉及接口、数据结构、环境变量或运行方式，还需要同步更新相关 `docs/` 文档；每次新增、修改或删除接口时，必须同步更新 `docs/api.md`。
...
| 模块 | 状态 | 当前结论 |
| --- | --- | --- |
| M01 账号、门店与权限 | 进行中 | 登录和基础鉴权已实现，管理能力未完成 |
| M02 审稿项目 | 未开始 | 暂无业务实现 |
...
| M12 部署、运维与开放接口 | 进行中 | 后端骨架、健康检查、API 文档和数据文档已完成，部署文档仍不完整 |
| M13 套餐、用量与账单 | 未开始 | MVP 暂不实现 |
| M14 AI 辅助能力 | 未开始 | MVP 暂不实现 |
...
- 门店员工可登录后台。
- 后台接口能识别当前用户和门店。
- 受保护接口无 Token 时返回 `401`。
- 用户停用或门店停用时不能继续访问。
- 门店管理、员工管理、角色管理实现后，本模块才可改为“已完成”。
...
- API 接口文档：`docs/api.md`，已记录当前已实现接口和后续计划接口分组。
