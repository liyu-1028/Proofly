...
| 模块 | 状态 | 当前结论 |
| --- | --- | --- |
| M01 账号、门店与权限 | 进行中 | 登录、基础鉴权和 M01 后端管理接口已实现，前端管理操作和完整权限模型未完成 |
| M02 审稿项目 | 未开始 | 暂无业务实现 |
...
- 前端员工创建、员工编辑、停用启用、重置密码和门店资料编辑功能，后续需要接入本次完成的后端接口。
...
- 当前门店资料更新、员工管理接口需要当前用户角色包含 `owner` 或 `admin`。
- 后端员工管理接口只允许操作当前登录用户所属门店的数据，不能跨门店访问。
...
- 用户停用或门店停用时不能继续访问。
- 前端员工管理、门店资料管理和必要的角色权限操作接入后，本模块可改为“已完成”。
